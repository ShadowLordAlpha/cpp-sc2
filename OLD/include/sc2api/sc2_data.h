...
namespace sc2
{
     ...
     * @brief Indicates if an ability is available, and if that ability requires a point.
     */
    struct AvailableAbility
    {
        const AbilityID abilityId; ///< Available ability ID
        const bool requiresPoint = false; ///< Indicates if the ability requires a point to invoke.
    };

    /**
     * @struct AvailableAbilities
     * @brief All available abilities for a unit.
     */
    struct AvailableAbilities
    {
    ...
    struct AbilityData
    {
        ...
        uint32_t remaps_to_ability_id = 0;
    ...
    struct Weapon
    {
        ...
        const float damage;
        const std::vector<DamageBonus> damageBonus; // Extra damage when attacking a unit of a certain attribute
        const uint32_t attacks; // Number of hits per attack. (eg. Colossus has 2 beams)
        const float range;
        const float speed; // Time between attacks
    };
