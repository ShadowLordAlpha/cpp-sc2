...
namespace sc2
{
    ...
    namespace
    {
        ...
        [[nodiscard]] std::optional<std::filesystem::path> GetEnvironmentPath(const char* env_var) {
        ...
        // Validate path with better error reporting
        void ValidateExecutablePath(const std::filesystem::path& path) {
            if (path.empty() || path.string().size() < 2) {
                throw ArgumentParsingError("Executable path is empty or too short");
            }

            if (!std::filesystem::exists(path)) {
                throw ExecutableNotFoundError(path.string());
            }
        }

        // Modern C++20 argument setup using designated initializers concept
        auto CreateFileArguments() {
            return std::vector{
                    Arg{"executable", {"-e", "--executable"}, "", true},
                    Arg{"realtime", {"-r", "--realtime"}, ""},
                    Arg{"port", {"-p", "--port"}, ""},
                    Arg{"map", {"-m", "--map"}, ""},
        ...
        auto CreateCommandLineArguments() {
            return std::vector{
                    Arg{"executable", {"-e", "--executable"}, "The path to StarCraft II."},
                    Arg{"step_size", {"-s", "--step_size"}, "How many steps to take per call."},
                    Arg{"port", {"-p", "--port"}, "The port to make StarCraft II listen on."},
                    Arg{"realtime", {"-r", "--realtime"}, "Whether to run StarCraft II in real time or not."},
                    Arg{"map", {"-m", "--map"}, "Which map to run."},
                    Arg{"timeout", {"-t", "--timeout"}, "Timeout for how long the library will block for a response."},
                    Arg{"data_version", {"-d", "--data_version"}, "Data hash of the game version to run (see versions.json)"}
            };
        }
    ...
    bool ParseFromFile(ProcessSettings &process_settings, GameSettings &game_settings, const std::string &file_name) {
        ...
        // Only update process_path if the executable argument was provided and valid
        if (auto executable = reader.get<std::filesystem::path>("executable")) {
            process_settings.process_path = executable.value();
        }

        // Use value_or to preserve existing settings if not specified in file
    ...
    bool ParseSettings(int argc, char *argv[], ProcessSettings &process_settings, GameSettings &game_settings) {
        ...
        process_settings.process_path = arg_parser.get<std::filesystem::path>("executable").value_or(
