...
bool TestReplayObservation(int argc, char** argv) {
    ...
    if (!observer.saw_camera() || !observer.camera_near_target()) {
        std::cerr << "TestReplayObservation: raw camera move missing from replay actions\n";
        success = false;
    }
    if (!observer.saw_chat_text()) {
        std::cerr << "TestReplayObservation: chat line never appeared in GetChatMessages\n";
        success = false;
    }
    if (!observer.chat_cleared()) {
        std::cerr << "TestReplayObservation: chat messages did not clear on later steps\n";
        success = false;
    }
    return success;
}
